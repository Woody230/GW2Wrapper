...
interface GetByQuantity<Model, Quantity> {
     ...
     */
    suspend fun byQuantity(quantity: Quantity, options: Gw2Options = Gw2Options.Default): GetResult<Model>

    /**
     * Gets the [Model] associated with the [quantity], or throws a [Gw2ClientException] if unable to fulfill the request.
     */
    suspend fun byQuantityOrThrow(quantity: Quantity, options: Gw2Options = Gw2Options.Default): Model

    /**
     * Gets the [Model] associated with the [quantity], or null if unable to fulfill the request.
     */
    suspend fun byQuantityOrNull(quantity: Quantity, options: Gw2Options = Gw2Options.Default): Model?
}