...
interface GetByInput<Model, Input> {
     ...
     */
    suspend fun byInput(input: Input, options: Gw2Options = Gw2Options.Default): GetResult<List<Model>>

    /**
     * Gets the [Model]s that require the given [input], or throws a [Gw2ClientException] if unable to fulfill the request.
     */
    suspend fun byInputOrThrow(input: Input, options: Gw2Options = Gw2Options.Default): List<Model>

    /**
     * Gets the [Model]s that require the given [input], or an empty list if unable to fulfill the request.
     */
    suspend fun byInputOrEmpty(input: Input, options: Gw2Options = Gw2Options.Default): List<Model>
}