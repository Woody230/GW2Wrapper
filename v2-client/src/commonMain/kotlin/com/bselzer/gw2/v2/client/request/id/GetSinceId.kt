...
interface GetSinceId<Model, Id, Value> where Id : Identifier<Value>, Model : Identifiable<Id, Value> {
    /**
     * Gets the [Model]s newer than the [Model] associated with the [id], or a [GetResult.Failure] if unable to fulfill the request.
     */
    suspend fun since(id: Id, options: Gw2Options = Gw2Options.Default): GetResult<List<Model>>
     ...
     */
    suspend fun sinceOrThrow(id: Id, options: Gw2Options = Gw2Options.Default): List<Model>

    /**
     * Gets the [Model]s newer than the [Model] associated with the [id], or an empty list if unable to fulfill the request.
     */
    suspend fun sinceOrEmpty(id: Id, options: Gw2Options = Gw2Options.Default): List<Model>
}